					...
					while (!isEmpty(stack))							
					{
							  ...
							  else														  //abs(ParentNode->BF)>1，调用4种旋转
							  {
										...
										else
										{
												  ...
												  if (Head->lchild == ptemp)	//左树为未平衡化之前ptemp结点(ParentNode的孩子结点)
												  {
															Head->lchild = ParentNode;				//链接新的根节点
												  }
												  else									  //右树为未平衡化之前ptemp结点(ParentNode的孩子结点)
												  {
															Head->rchild = ParentNode;				//链接新的根节点
												  }
